        ...
        for table in tables:
            for record in table.records:
                if record.get_time() > last_processed_time_b:
                    ...
                    if event == "THEFT_ALARM":
                        print(f"Contattare le autorità, {bike_id} possibile furto")

                    if event == "OUT_OF_BOUNDS":
                        print(f"Notificare il noleggiatore di {bike_id} rientrare in zona")
